...
def set_character_status_console_message_id(message_id, channel_id):
    console_ref.child(CONSOLE_KEY_CHARACTER_STATUS_CHANGE).update(
        {
            CONSOLE_KEY_MESSAGE_ID: f'{message_id}',
            CONSOLE_KEY_CHANNEL_ID: f'{channel_id}'
        }
...
def get_character_status_console_message_id() -> Optional[str]:
    data = console_ref.get()
    return data[CONSOLE_KEY_CHARACTER_STATUS_CHANGE][
        CONSOLE_KEY_MESSAGE_ID] if data is not None and CONSOLE_KEY_CHARACTER_STATUS_CHANGE in data else None


def get_character_status_console_channel_id() -> Optional[str]:
    data = console_ref.get()
    return data[CONSOLE_KEY_CHARACTER_STATUS_CHANGE][
        CONSOLE_KEY_CHANNEL_ID] if data is not None and CONSOLE_KEY_CHARACTER_STATUS_CHANGE in data else None
