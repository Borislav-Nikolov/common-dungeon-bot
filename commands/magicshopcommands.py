...
async def handle_shop_commands(message, client):
    ...
    if keywords[0] == shop_key and botutils.is_shop_channel(message):
        ...
        # ADMIN COMMANDS
        if command_message == 'generate' and botutils.is_admin(message):
            await handle_generate(message, character_levels_csv=keywords[2])
        elif command_message == 'refresh' and botutils.is_admin(message):
            await handle_refresh(message)
        elif command_message == 'repost' and botutils.is_admin(message):
            await handle_repost(message)
        elif command_message == 'sell' and not keywords[2].isnumeric() and botutils.is_admin(message):
